...
public class ControllerTest {
    ...
    @Test
    public void returnAllMoviesOnSearchSuccess() throws Exception, MovieNotFoundException {
        when(searchService.searchAllMoviesBasedOnKeyword(any())).thenReturn(Arrays.asList(movie));

        mockMvc.perform(get("/api/v2/movies/"+any())//making dummy http post request
        ...
        verify(searchService, times(1)).searchAllMoviesBasedOnKeyword(any());

    }

    @Test
    public void returnAllMoviesOnSearchFailure() throws Exception, MovieNotFoundException {
        when(searchService.searchAllMoviesBasedOnKeyword(any())).thenReturn(Arrays.asList());
    ...
    @Test
    public void returnMoviesOnSearchUSingFilterSuccess() throws Exception, MovieNotFoundException {
        when(searchService.searchAllMoviesBasedOnKeywordAndGenre(any(),any())).thenReturn(Arrays.asList(movie));

        mockMvc.perform(get("/api/v2/movies/"+any()+"/"+any())//making dummy http post request
                        .contentType(MediaType.APPLICATION_JSON)//setting the content type of the post request
                        .content(jsonToString(movie)))//firstly, java object will be converted to json string then will  be passed with the mock http request.
                .andExpect(status().isOk()).andDo(MockMvcResultHandlers.print());
        verify(searchService, times(1)).searchAllMoviesBasedOnKeywordAndGenre(any(),any());
